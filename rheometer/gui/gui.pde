...
int[] DAC1 = new int[1024], A0 = new int[1024];
;//c = new int[1024];
float freq_val; 
...
class menu_button {
  ...
  void draw_button() {
    ...
    if (check_mouse_on_button()) {
      button_colour = color(200, 200, 200);
    } else {
      button_colour = color(150, 150, 150);
    }
  ...
  if (key == '\n' ) {
    ...
    if (type_frequency.box_selected) { //covert frequency to timer counter understandble value
      ...
      if ( 0.1 <= freq_val && 20 >= freq_val ) {
        ...
        if (2.0 >= freq_val) {
          interval = (width - 2*x_offset)/32;
        } else if (2.0 < freq_val && 6.0 >= freq_val) {
          interval = (width - 2*x_offset)/8;
        } else if (6.0 < freq_val) {
          interval = (width - 2*x_offset)/4;
        }
      }
   ...
   System.out.println( " val_in[0] = " + val_in[0]
    ...
    //typing = typing + key;
    if ( (typing.length() - 1) == typing_cursor) {
      typing = typing + key;
    ...
    if (key == 8) {//ASCII 08 should be backspace
      if (0 >= typing_cursor) {
        LHS = "";
      } else if (0 <= typing_cursor) {
        LHS = typing.substring(0, typing_cursor);
      } 
...
void mousePressed() {
  ...
  if (eqcentre.check_mouse_on_button()) {
    ...
    serialPort.write(val_in[0]);
    System.out.println("val_in[0] = " + val_in[0]);
    if (eqcentre.button_activated) {
      eqcentre.button_text = "per";
    } else {
...
void initialise_interface() {
  ...
  ACDC.y_0 = 40;
  ACDC.x_width = 20;
  type_file_name.x_0 = 560;
  type_file_name.y_0 = 10;
  type_file_name.x_width = 160;
  type_frequency.x_0 = 340;
  type_frequency.y_0 = 10;
  type_set_strain.x_0 = 340;
  type_set_strain.y_0 = 40;
  type_set_simu.x_0 = 560;
  type_set_simu.y_0 = 100;
