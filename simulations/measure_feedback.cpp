...
int dmudt, darraydt[range], DC_func = 2047, NR = 0, korb = 0, simu_k = 100000, simu_b = 1000, Torv = 0;
...
unsigned long int t_i = 0, t_f = 0, val = 31818;//35000;//175000*4; //does this correspond to what the gui says when turned on? check - NO
...
void TC0_Handler() {
  pos = analogRead(measure); //reads the voltage at analog pin A0 - simulate by replacing function with something that gets the theta value from the Python program 
  //(perhaps with some delay) and mapping to a value between 
