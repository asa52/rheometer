#ifndef CODE_FEEDBACK_FUNCS_H
#define CODE_FEEDBACK_FUNCS_H
...
void handle_const_strain_feedback() {
    ...
    if ((pos <= 1558 || pos >= 3633) && amp >= 1) {
        ...
        if (1 == freq_check) {
            if ((pos + 16) >= pos_rec &&
                (pos - 16) <= pos_rec) {//points are sparse,
                // so a strict recurrence condition can miss many cycles
                if (175000 <=
                    val) {//at freq < 2Hz shorter periods are presumably the 2Hz fundamental
                    if (6 < t_diff) {
                        if (0 == (f_count % 2) && 1 < f_count
                            && 15 >
                               (t_diff + rec_times[f_count])) { f_count -= 2; }
                        else if (0 == (f_count %
                                       2)) {//2nd recurrence of a value implies completed period
                    ...
                    t_diff = 0;
                           ...
                           val) {//at freq > 2Hz reject periods shorter than half a driving cycle
                    if (6 < t_diff) {
                        if (0 == (f_count % 2) && 1 <= f_count
                            && 60 >
                               (t_diff + rec_times[f_count])) { f_count -= 2; }
                ...
                if (16 == A0_period_count) { A0_period_count = 0; }
                       ...
                       t_diff) {//handle time overflow for non-recurring pos values
            ...
            switch (p) {
                ...
                case 11 :
                    if (0 > phase_estimates[delta_num]) {
                        send_3_byte_value(phase_estimates[delta_num],
                                          0b00010001);
                    } else {
                        send_3_byte_value(phase_estimates[delta_num],
                                          0b00010000);
        ...
        if (p < range) {
            ...
            if (0 < p &&
                range > p) {// time differential quotient (new - old) / dt
         ...
         dmudt = ((array[p-1] - array[range-1])/dt);
            ...
            if ((p % sample_num) ==
                1) {// reset estimates every full cycle, for p == 1, 121, 241 and 361
                ...
                if (0 <= e_num) {
                    ...
                    A0_amp = ((peaksnt[feed_num][0] - troughsnt[feed_num][0]) /
                              2);
                    dA0dt_amp = (
                            (peaksnt[feed_num][2] - troughsnt[feed_num][2]) /
                            2);
                    // positive phase_estimate means that the response leads the driving

                    if (0 == (delta_num %
                              2)) { // even numbered elements are estimates from peaks
                        phase_estimates[delta_num] = (30 - t_peak);
                        if (-60 > phase_estimates[delta_num]) {
                            phase_estimates[delta_num] = sample_num +
                ...
                if (array[p] >
                    array[p - 1]) { //start making new extremum estimates
                    peak = array[p];
                    trough = array[p - 1];
                    t_peak = t;
                    t_trough = last_t;
            ...
            } else if (1 < p && (p % sample_num) !=
                                1) { //else compare values within cycle
                if (peak < array[p]) {
                    peak = array[p];
                    t_peak = t;
                }
    ...
    if (0 == run_option) {
        ...
        if (0x7ff == waveformsTable[t] && 0 <= feed_num) {
            ...
            if (((4 * mu_tol) >= sym_check && ((-4) * mu_tol) <= sym_check)
                ...
                if (0 ==
                    strain_closing_in) { adaptive_step_calculation_for_const_strain(); }
                ...
                if ((((peaksnt[feed_num][0] - troughsnt[feed_num][0]) / 2) <=
                     set_strain - mu_tol) && 2048 >= (amp + amp_step)) {
                    amp += amp_step;
                ...
                else if ((((peaksnt[feed_num][0] - troughsnt[feed_num][0]) /
                           2) >= set_strain + mu_tol) &&
                         0 <= (amp - amp_step)) {
                    amp -= amp_step;
                    old_amp_step = amp_step;
...
//( (waveformsTable[t] * 512 * amp) / 1048576 )
    ...
    if (0 != old_amp_step && 1 <= feed_num) {
        ...
        if (0 > step_estimate) {
            if (0 < step_count && 0 < past_step_estimates[step_count -
                                                          1]) { sign_change_count++; }

            step_estimate = (-step_estimate);

        } else if (0 < step_count && 0 > past_step_estimates[step_count -
                                                             1]) { sign_change_count++; }

        if ((2048 >= (amp + step_estimate)) && (0 <= (amp - step_estimate))) {
            amp_step = step_estimate;

        } else if ((2048 >= (amp + (step_estimate / 2))) &&
                   (0 <= (amp - (step_estimate / 2)))) {
            amp_step = (step_estimate / 2);
        } else if ((2048 >= (amp + (step_estimate / 4))) &&
                   (0 <= (amp - (step_estimate / 4)))) {
        ...
        if (((((peaksnt[feed_num][0] - troughsnt[feed_num][0]) / 2) <=
              set_strain - mu_tol) && 2048 > amp)
            || ((((peaksnt[feed_num][0] - troughsnt[feed_num][0]) / 2) >=
                 set_strain + mu_tol) && 0 < amp)) { amp_step = 1; }
    ...
    if (8 == step_count) {
        if (3 <= sign_change_count) {
            ...
            for (int i = 0; i < step_count; i++) {
                ...
                d_amp += past_amps[i];
                if (1 <= i) {
                    max_amp_step = max(past_amp_steps[i],
                                       past_amp_steps[i - 1]);
                    min_amp_step = min(past_amp_steps[i],
                                       past_amp_steps[i - 1]);
                }
            }
...
void settle_amp() {
    ...
    if (set_strain <
        ...
        || (-1) * set_strain > (2 * set_strain - (peaksnt[feed_num][0] -
                                                  troughsnt[feed_num][0]))) { strain_closing_in = 0; }
