...
int speed = 115200;
int bytesReceived = 0;
int c1, c2, c3, i, j, amp = 128, trough, centre, peak, peak_to_peak; 
int A0_period_estimate, phase_estimate, sym_check;
int t_d = 0, t_a = 0, t = 0, interval = 20, val, pos_d, pos_a, x_offset = 60;
...
String saved = "", debug = "";
String file_name = "", set_strain_text = "0", frequency_text = "0.5", set_stress_text = "";
String set_simu_k_text = "0", set_simu_b_text = "0", set_DC_text = "2047", set_centre_text = "";

static int sample_num = 120;


//static int[] waveformsTable = new int[sample_num]; 
...
void draw() {
     ...
     DAC1[t_d] = ( ( (waveformsTable[t] - waveformsTable[0]) * amp) / 2048 ) + 2047;}
     else if(NRonoff.button_activated){
     DAC1[t_d] = ( ( (waveformsTable[t] - waveformsTable[0]) * amp) / 2048 ) 
         //+ ( (mu - 2047)  * k_val ) / (128)
         //+ ( ( ( (last_mu -2047) + (mu - 2047) ) * k_val ) / (128*2)) 
         + ( ( ( (twolast_mu - 2047) + (last_mu - 2047) + (mu - 2047) ) * k_val ) / (128*3)) 
         //+ ( ( dmudt * 512 * simu_b ) / (simu_b_unit * 512) ) ) 
         + 2047 ;
       }
...
void delay_micros(int delay)
{
  long time = System.nanoTime(); 
  while( (System.nanoTime() - time) <= (delay*1000));
}
...
void oscilloscope(){
   ...
   point(x_offset + j, (top_strain_pixel + y_half_axis_pixel) - ((A0[j]-2048)/16));
   }
...
void initialise_interface(){
  ...
  type_set_strain.constant_text = set_strain_text + " DAC1";
  type_set_simu.constant_text = set_simu_k_text + " kDAC1eq";
